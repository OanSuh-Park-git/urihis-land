...
function Input({ value, onChange, placeholder, className = "" }: {
  ...
  return (
    <input
      ...
      className={`w-full bg-transparent border border-parchment/20 text-parchment px-3 py-2 font-sans text-sm focus:outline-none focus:border-secondary placeholder:text-parchment/30 ${className}`}
    />
  );
}
...
export default function AdminPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        console.error("[Admin] Firestore error:", msg);
        setFetchError(msg);
      } finally {
        setFetching(false);
      }
  ...
  async function handleAddArticle(e: React.FormEvent) {
    ...
    try {
      await addDoc(collection(db, "articles"), { ...form, createdAt: serverTimestamp() });
      setForm(EMPTY_FORM);
      setFormSuccess(true);
      await loadArticles();
    ...
    <div className="min-h-screen bg-primary px-4 sm:px-6 py-16">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-10">
          ...
          <a href="/" className="font-sans text-xs text-parchment/40 border border-parchment/20 px-4 py-2 hover:border-secondary/50 hover:text-secondary transition-colors">
            ← 홈으로
          </a>
        </div>
        ...
        {fetchError && (
          <div className="mb-6 border border-accent/40 bg-accent/10 px-4 py-3 text-accent font-sans text-sm">
            ...
            <p className="text-xs mt-1 text-accent/70">
              Firebase 콘솔 → Firestore → 규칙 탭에서 읽기 권한을 확인해 주세요.
            </p>
          </div>
        )}
                ...
                <tbody>
                  {subscribers.length === 0 ? (
                    ...
                    <tr key={s.id} className="border-b border-parchment/5 hover:bg-parchment/5">
                      ...
                      <td className="px-4 py-3 text-parchment/30 text-xs font-mono truncate max-w-[120px]">{s.uid ?? "비로그인"}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        {tab === "articles" && (
          <div className="space-y-8">
            ...
            <div className="border border-secondary/20 p-6">
              <h2 className="text-parchment font-bold mb-5" style={{ fontFamily: "var(--font-nanum-myeongjo), serif" }}>
                새 기사 추가
              </h2>
              <form onSubmit={handleAddArticle} className="space-y-4" noValidate>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    <Label>날짜 (예: 2026.03.08)</Label>
                    <Input value={form.date} onChange={(v) => setForm({ ...form, date: v })} placeholder="2026.03.08" />
                  </div>
                </div>

                <div>
                  <Label>제목 *</Label>
                  <Input value={form.title} onChange={(v) => setForm({ ...form, title: v })} placeholder="기사 제목" />
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    <Label>읽기 시간 (예: 8분)</Label>
                    <Input value={form.readTime} onChange={(v) => setForm({ ...form, readTime: v })} placeholder="8분" />
                  </div>
                </div>

                {formError && <p className="font-sans text-xs text-accent">{formError}</p>}
                {formSuccess && <p className="font-sans text-xs text-secondary">기사가 추가되었습니다.</p>}
            ...
            <div className="border border-parchment/10 overflow-x-auto">
              <table className="w-full font-sans text-sm">
                <thead>
                  <tr className="border-b border-parchment/10 bg-parchment/5">
                    {["카테고리", "제목", "날짜", "링크", ""].map((h, i) => (
                      <th key={i} className="text-left px-4 py-3 text-parchment/50 font-normal text-xs">{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {articles.length === 0 ? (
                    <tr><td colSpan={5} className="px-4 py-10 text-center text-parchment/30 text-xs">등록된 기사가 없습니다.</td></tr>
                  ) : articles.map((a) => (
                    <tr key={a.id} className="border-b border-parchment/5 hover:bg-parchment/5">
                      ...
                      <td className="px-4 py-3 text-parchment max-w-[200px]">
                      ...
                      <td className="px-4 py-3 text-xs max-w-[160px]">
                        <a href={a.url} target="_blank" rel="noopener noreferrer"
                          className="text-secondary/70 hover:text-secondary underline truncate block">
                          {a.url}
                        </a>
                      </td>
                      <td className="px-4 py-3">
                        <button onClick={() => handleDelete(a.id)}
                          className="font-sans text-xs text-accent/60 hover:text-accent transition-colors cursor-pointer whitespace-nowrap">
                          삭제
                        </button>
                      </td>
                    </tr>
